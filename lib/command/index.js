...
app.get('/command', function (req, res) {
    var options = {
        ...
        command_data: req.query.command_data || NOT_SET,
        timestamp: parseInt(req.query.timestamp) || NOT_SET
    };
    check_not_set(options, function () {
        amb_manager.command(options, function (err, data) {
            if (err) {
                throw err;
                return;
            }
            res.send('command ' + options.command_data +
                     ' was given to ' + options.node_addr);
        });
    });
});
...
app.get('/command/timingevent', function (req, res) {
    ...
    check_not_set(options, function () {
        amb_manager.command_timing_event(options, function (err, data) {
            if (err) {
                throw err;
                return;
            }
...
app.get('/command/next/timingevent', function (req, res) {
    ...
    check_not_set(options, function () {
        amb_manager.command_next_timing_event(options, function (err, data) {
            if (err) {
                throw err;
                return;
            }
